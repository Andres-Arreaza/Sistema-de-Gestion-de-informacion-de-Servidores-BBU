        ...
        <div className="list-container">
            ...
            <div className="servicio-grid">
                {estatus && estatus.length > 0 ? (
                    estatus.map((est) => (
                        <div key={est.id} className="servicio-card">
                            <div className="servicio-card-header">
                                <strong className="servicio-nombre">{est.nombre}</strong>
                                <div className="servicio-acciones">
                                    <button className="accion-btn editar-btn" onClick={() => onEdit(est)}>
                                        <EditIcon />
                                    </button>
                                    <button className="accion-btn eliminar-btn" onClick={() => confirmarEliminacion(est)}>
                                        <TrashIcon />
                                    </button>
                                </div>
                            </div>
                            <p className="servicio-descripcion">{est.descripcion || 'Sin descripción'}</p>
                        </div>
                    ))
                ) : (
