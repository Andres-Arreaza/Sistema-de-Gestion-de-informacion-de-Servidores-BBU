...
const AmbienteFormModal = ({ isOpen, onClose, onSave, ambiente, ambientesExistentes }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const ambienteDuplicado = ambientesExistentes.find(
            a => a.nombre.toLowerCase() === nombreNormalizado && a.id !== ambiente?.id
        );
        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content-servicio" onClick={(e) => e.stopPropagation()}>
                <div className="modal-header">
                    <h2 className="modal-title">{ambiente ? 'Editar Ambiente' : 'Crear Nuevo Ambiente'}</h2>
...
export const Ambiente = () => {
    ...
    const handleSaveAmbiente = (formData) => {
        const esEdicion = !!ambienteSeleccionado;
        const metodo = esEdicion ? "PUT" : "POST";
        const url = esEdicion
            ? `${process.env.BACKEND_URL}/api/ambientes/${ambienteSeleccionado.id}`
        ...
        fetch(url, {
            ...
            .then(() => {
                ...
                Swal.fire({
                    icon: 'success',
                    title: '¡Éxito!',
                    text: `Ambiente ${esEdicion ? 'actualizado' : 'creado'} correctamente.`,
                    timer: 3000,
                                ...
                                <div key={ambiente.id} className="servicio-card">
                                    <div className="servicio-card-header">
                                        ...
                                        <div className="servicio-acciones">
                                            <button className="accion-btn editar-btn" onClick={() => handleOpenModal(ambiente)}>
                                                <EditIcon />
                                            </button>
